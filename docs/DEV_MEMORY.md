...
## 33. 2026-06-21 追加：第一月跑测补丁与完整月测

本轮接续“先修体验和基础系统”的路线，先补了几个会影响第一月跑测判断的基础问题，再从新开局跑完第一个月，截图保存到 `res://_qa_runtime_screenshots/`，文件名为 `month1_01_opening.png` 到 `month1_49_after_month_settle_next_story.png`。

代码改动：
- `scenes/MainGame.gd`
  - 新增 `_pending_deferred_result_dialogs`，让 `show_result_text()` 的 deferred 结果弹窗在真正出现前也算阻塞，避免工作日套餐面板抢在结算页之前弹出。
  - 新增 `_pending_app_unlock_messages` 和 `_maybe_show_pending_app_unlock_message()`，App 解锁通知排队到周末、无阻塞层时再显示。
  - 锁定/未播报解锁的 App 继续隐藏，避免日记和 BOSS 直聘无介绍地一起冒出来。
- `scripts/GameManager.gd`
  - 新增 `has_announced_app_unlock(app_id)`，给手机桌面判断“已解锁但是否已向玩家介绍过”。
- `scripts/TutorialDirector.gd`
  - 第 2 周和第 3 周地图提示改为按 `month:week` 记录，避免第 2 周提示后第 3 周新增地点提示被吞。
  - 第 3 周新增提示文案：地图开放图书馆、咖啡厅和健身房，它们改变可去场所和可能遇见的人。
- `scripts/LocationActionRunner.gd`
  - `home` 地点临时使用 `res://Assets/title_bg2.png`，避免“宅家刷手机”黑屏/纯色。
- `scripts/EnvironmentController.gd`
  - 临时地点 fallback 色块整体提亮，酒吧、咖啡、夜市、加班等未做美术地点至少不再像黑屏。

验证结果：
- Godot 编译通过：`MainGame.gd`、`GameManager.gd`、`TutorialDirector.gd`、`LocationActionRunner.gd`、`EnvironmentController.gd`。
- 第 2 周工作日林凡剧情结束后，工作日套餐面板不会和剧情/结算重叠；结果页点掉后一帧内面板正常出现。
- 第 2 周地图提示出现，地图 App 发光；第 3 周地图提示也出现，`_early_map_hint_shown_weeks` 记录了 `1:2` 和 `1:3`。
- 日记 App 在 `turn_count=5` 前不会提前显示；进入第 2 月第 1 周后会通过解锁通知进入介绍队列。
- “宅家刷手机”真实周末路径可执行：占用周末日程槽、显示家里背景、结算后回到可操作周末。

第一月跑测路线：
- 第 1 周：支付宝教程 -> 地图加班 -> 微信家庭群情绪 +8 -> 数值蓝字教程 -> 深圳湾散心 -> 结束本周。
- 第 2 周：林凡第一次擦肩 -> 中档饮食 + 正常打卡 -> 周末宅家刷手机 + 夜市大排档。
- 第 3 周：中档饮食 + 摸鱼 -> 周末图书馆 + 健身房。
- 第 4 周：中档饮食 + 正常打卡 -> 周末公司加班 + 宅家刷手机 -> 月底账单 -> 进入第 2 月第 1 周。

跑测结论：
- 第一周新手链路现在基本能走通，支付宝、地图、微信、数值面板、海边、结束本周形成了一个完整闭环。
- 海边和图书馆的美术显著提升沉浸感；健身房雨天背景也成功出现。加班、夜市、咖啡等无美术地点仍只能靠色块和文案撑，沉浸感弱。
- 支付宝教程小框位置正确，不再遮挡核心财务数值；但支付宝提示字和地图卡片字仍偏小。
- 微信家庭群 +8 事件能结算，但入口不够明显：聊天页只有气泡和底部“+”，玩家可能不知道要点哪里。
- 家庭群事件结算时仍有多层 UI 堆叠感：前台结算清楚，但微信灰层还压在中层。后续应统一“手机内事件触发结算时压暗手机层、突出前台对话”。
- 第 3 周阿强伏笔已经写入家庭群并产生未读，但前台只是一句家庭群新消息，作为主线/半主线人物铺垫太弱，玩家大概率会错过。
- 工作日套餐界面不再和剧情撞层，但界面仍像系统菜单，后续要做成更像“本周生活计划”的呈现。
- 月底账单信息完整，但正文偏小、偏财务报表。后续应改为“摘要优先，明细可展开”，先告诉玩家本月活下来没有、哪里最伤，再给账单细节。

下一步建议：
- 优先修第一月发现的体验问题，而不是继续加新系统：
  1. 微信家庭群事件入口做明显：家庭群内提供醒目的“看看家人消息/回应一下”按钮，不依赖底部加号。
  2. 手机内事件结算统一压暗中层 UI，解决微信/支付层和 galgame 结算叠屏的观感。
  3. 第 3 周阿强伏笔改为强提示：前台说“家庭群有新消息”，并引导玩家打开微信看，不要只是悄悄未读。
  4. 地图卡片和支付宝教程继续放大关键文字。
  5. 月底账单改版为摘要卡 + 明细。

## 34. 2026-06-21 追加：先冻结微信，只改第 5 点“月底账单”

用户明确要求：微信系统先不动，微信是后续要详细讨论 UI 和系统的大工程；本轮只处理上一轮跑测建议里的第 5 点，即“月底账单改版为摘要卡 + 明细”。

代码改动：
- `scenes/MainGame.gd`
  - 保留所有月底结算公式和 `GameManager.confirm_monthly_settlement()` 流程，不改数值逻辑。
  - `_build_month_end_bill_text()` 从长篇财务报表改成短明细：现金 + 工资、房租/餐饮/花呗最低、实际扣款、结算后现金、结算后总债、利息和风险提示。
  - 新增 `_get_month_end_verdict()`，按结算后现金、花呗最低是否失败、情绪压力、总债务生成玩家先看到的结论：如“第一张账单稳住了”“勉强撑过这个月”“现金断了”等。
  - 新增动态 UI 渲染函数 `_render_month_end_bill()`：顶部结论卡，中间三张大数字卡，底部账单明细，最后是醒目的黄色确认按钮。
  - 新增月底卡片/文字/按钮样式 helper：`_make_month_end_card()`、`_make_month_end_label()`、`_make_month_end_stat_card()`、`_style_month_end_confirm_button()`。

验收：
- Godot 编译：`res://scenes/MainGame.gd` 通过。
- 运行态直接造第一月底结算数据：现金 4060、工资 3800、房租 1500、餐饮 2400、花呗 2876。
- 截图保存：
  - `res://_qa_runtime_screenshots/month_end_bill_redesign.png`
  - `res://_qa_runtime_screenshots/month_end_bill_redesign_v2.png`
- v2 截图显示：标题“月底账单”、顶部结论“第一张账单稳住了”、三张数字卡“结算后现金 3473 / 本月扣款 -4387 / 结算后债务 2508”、底部明细和黄色确认按钮。
- 点击确认按钮后成功进入第 2 月第 1 周：现金 3473，花呗债务 2508，`awaiting_month_settle=false`，月底弹窗关闭。

注意：
- 本轮没有碰微信系统，也没有处理微信家庭群入口、微信叠屏、阿强提示等问题。
- MCP `execute_game_script` 中使用 `await get_tree().process_frame` 时会触发 MCP runtime 警告，但没有游戏脚本错误；确认按钮实际已经正常生效。

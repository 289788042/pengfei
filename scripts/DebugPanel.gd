...
func _build_scene_tab() -> void:
	...
	_add_scene_btn("跳过开场 → 直接进入周末", func():
		GameManager.week_in_month = 1
		get_tree().change_scene_to_file("res://scenes/MainGame.tscn")
	)

	_add_scene_btn("跳到第2周", func():
		GameManager.week_in_month = 2
		get_tree().change_scene_to_file("res://scenes/MainGame.tscn")
	)

	_add_scene_btn("跳到第3周（月结算前）", func():
		GameManager.week_in_month = 3
		get_tree().change_scene_to_file("res://scenes/MainGame.tscn")
	)
	...
	_add_scene_btn("重置所有数据", func():
		...
		GameManager.npc_database.clear()
		GameManager.week_in_month = 1
	...
	_add_scene_btn("解锁全部微信NPC", func():
		for npc_id in ["family_group", "lin_fan", "chen_yu"]:
...
func _build_stat_tab() -> void:
	_add_label("女主角属性调节：", 15, Color(0.7, 0.9, 1.0))
	_add_spacer()
	...
	_add_spacer()
	_add_label("当前周: 第%d周" % GameManager.week_in_month, 14, Color(0.8, 0.8, 0.8))
	...
	for npc in GameManager.npc_database:
		...
		if req.size() > 0:
			...
			for stat_name in req:
				var req_val: int = int(req[stat_name])
				var cur_val: int = GameManager.get(stat_name) if stat_name != "money" else GameManager.money
				var ok: bool = cur_val >= req_val
				var color_mark: String = "✓" if ok else "✗"
				cond_text += "%s ≥ %d (%s%d) %s  " % [stat_name, req_val, "" if ok else "", cur_val, color_mark]
			_add_label("  条件: " + cond_text, 12, Color(0.6, 0.8, 0.6) if req.size() > 0 else Color(0.7, 0.7, 0.7))
		...
		if milestones.size() > 0:
			_add_spacer()
			_add_label("  里程碑：", 12, Color(0.8, 0.8, 0.8))
			for ms in milestones:
				var ms_title: String = ms.get("title", "")
				var ms_aff: int = int(ms.get("trigger_affection", 0))
				var ms_req: String = ms.get("req_stat", "")
				var ms_req_val: int = int(ms.get("req_val", 0))
				_add_label("    ·好感≥%d: %s (需%s≥%d)" % [ms_aff, ms_title, ms_req, ms_req_val], 11, Color(0.6, 0.6, 0.6))

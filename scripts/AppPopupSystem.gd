...
var _overlay_builder: RefCounted
...
func init(main: Node) -> void:
	...
	_overlay_builder = _new_overlay_builder()
...
func _new_overlay_builder() -> RefCounted:
	var script := ResourceLoader.load("res://scripts/PhoneAppOverlayBuilder.gd", "", ResourceLoader.CACHE_MODE_REPLACE) as Script
	var builder := script.new() as RefCounted
	builder.init(self)
	return builder
...
func _build_app_overlay(parent: ColorRect, title: String, top_color: Color, subtitle: String, items: Array) -> void:
	if _overlay_builder and _overlay_builder.has_method("build_app_overlay"):
		_overlay_builder.build_app_overlay(parent, title, top_color, subtitle, items)
